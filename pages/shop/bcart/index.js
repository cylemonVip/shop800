...
Page({
    ...
    onShow:function(options){
      ...
      // _function.getCartList(wx.getStorageSync("utoken"),that.initgetCartListData,this)
        var info = Mock.getCartList();
        this.initgetCartListData(info);
    },
    ...
    bind_cart_number_jian:function(e){
        ...
        for(var i=0;i<datas.length;i++){
            if(datas[i].id == this_cart_id){
                ...
                var info = Mock.editCartList();
                that.initeditCartListData(info);
            }
        }
      that.setData({
        all_goods_price:0,
    ...
    initeditCartListData:function(data){
      console.log("initeditCartListData = " + JSON.stringify(data));
        var that = this;
        if(data.code == 1){
